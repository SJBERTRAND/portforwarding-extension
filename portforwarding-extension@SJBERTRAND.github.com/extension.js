...
const SSHServerConnection = class {
    ...
    const CommandArray = [];
        if (_settings.get_boolean('password-required'+_number) == true) {
            CommandArray.push('sshpass', '-p', _settings.get_string('server-password'+_number), 'ssh' , '-p' , _settings.get_string('ssh-port'+_number) ,'-N' , '-o', 'ConnectTimeout=10', '-o', 'StrictHostKeyChecking=accept-new', '-o', 'UpdateHostKeys=yes');
        }else{
            CommandArray.push('ssh' , '-p' , _settings.get_string('ssh-port'+_number) ,'-N', '-o', 'PasswordAuthentication=no' , '-o' , 'BatchMode=true', '-o', 'ConnectTimeout=10', '-o', 'StrictHostKeyChecking=accept-new', '-o', 'UpdateHostKeys=yes');
        };
